    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = async (e) => {
            try {
                ...
                for (let i = 0; i < Math.min(10, jsonData.length); i++) {
                    ...
                    if (Array.isArray(row)) {
                        ...
                        const matches = keywords.filter(keyword =>
                            row.some(cell =>
                                cell && cell.toString().toUpperCase().includes(keyword)
                            )
                        );
                ...
                if (headerRowIndex === -1) {
                    throw new Error('Cabeçalho não encontrado. Verifique se o arquivo contém as colunas: ESCRITURA, LIVRO, FOLHA, OUTORGANTE');
                }
                ...
                const findColumn = (keywords) => {
                    for (const keyword of keywords) {
                        const index = headers.findIndex(h =>
                            h && h.toString().toUpperCase().includes(keyword.toUpperCase())
                        );
