...
export function TiposEscritura() {
  ...
  const tiposFiltrados = tipos.filter((t) => t.nome.toLowerCase().includes(filtro.toLowerCase()));

  return (
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
      <div style={{ marginBottom: '1.5rem' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--text-primary)' }}>Tipos de Escritura</h2>
                ...
                <tbody>
                  {tiposFiltrados.map((tipo) => (
                    <tr key={tipo.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                      <td style={{ padding: 'var(--spacing-md)' }}>{tipo.nome}</td>
